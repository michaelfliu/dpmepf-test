...
class ImagenetNumpyDataset(pt.utils.data.Dataset):
    ...
    def npz_to_npy_batches(train_data_root='../data/Imagenet32_train_npz/',
        ...
        for data_id in range(1, 11):
            ...
            np.save(os.path.join(train_data_root, f'train_data_batch_{data_id - 1}_y.npy'), data['labels'] - 1)
            if data_id == 1:
        ...
        if val_data_root is not None and os.path.exists(val_data_root):
            data = np.load(os.path.join(val_data_root, 'val_data.npz'))
            np.save(os.path.join(val_data_root, 'val_data_x.npy'), data['data'])
            np.save(os.path.join(val_data_root, 'val_data_y.npy'), data['labels'] - 1)
...
def load_imagenet_numpy(data_root, batch_size, workers, test_set=False, img_hw=32):
    ...
    # Create the dataloader
    dataloader = pt.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=workers)
    return dataloader
