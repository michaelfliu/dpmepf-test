import os
import torch as pt
from dp_mepf_args import (
    get_args,
    get_imagenet_norm_min_and_range,
    get_param_group_tuples,
)
...
from feature_matching import (
    adam_moving_average_update,
    extract_and_bound_features,
    compute_data_embedding,
    sort_feats_by_label,
    get_test_data_embedding,
    hybrid_labeled_batch_embedding,
    l2_norm_feat_analysis,
    get_static_losses_valid,
...
def get_real_data_embedding(
    ...
    if ckpt is None:  # compute true data embeddings
        emb_res = compute_data_embedding(
            encoders,
            n_matching_layers,
            device,
            train_loader,
        ...
        if isinstance(encoders, Encoders) and do_embedding_norms_analysis:
            l2_norm_feat_analysis(
                feat_emb.real_means,
                feat_emb.real_vars,
                encoders.n_feats_by_layer,
                encoders.layer_feats,
                writer,
            )
        if dp_params.noise is not None:
            dp_res = dp_dataset_feature_release(
                feat_emb, feat_l2_norms, dp_params, matched_moments, n_samples, writer
            )
    ...
    if ckpt is None:
        ...
        labeled_val = False
        test_emb = get_test_data_embedding(
            val_encoders,
            n_matching_layers,
            device,
            channel_ids_by_enc,
...
def get_fake_data_embedding(
    ...
    ef_res = extract_and_bound_features(
        fake_data,
        encoders,
        n_matching_layers,
        match_with_top_layers,
    ...
    if isinstance(encoders, Encoders) and encoders.n_split_layers is not None:
        ...
        ff, ff_sq = hybrid_labeled_batch_embedding(
            encoders,
            feats_batch,
            feats_sqrd_batch,
            l2_norms,
        ...
        if n_classes is not None:
            ...
            if feat_emb.fake_feats_sqrd is not None:
                feat_emb.fake_feats_sqrd = sort_feats_by_label(
                    feat_emb.fake_feats_sqrd, gen_labels
                )
...
def delete_old_syn_data(
    syn_data_file, old_syn_data_file, best_proxy_result, step, keep_best_syn_data
):
...
def update_best_score(
    eval_score, step, syn_data_file, dataset, best_result: BestResult
):
    lower_is_better = False if dataset in {"dmnist", "fmnist"} else True
    score_is_better = best_result.score is None or (
        (eval_score < best_result.score) == lower_is_better
    )
...
def update_best_proxy_score(
    proxy_score, step, syn_data_file, best_proxy_result: BestResult
):
...
def get_enc_input_scalings(
    net_enc_type, dataset, image_size, data_scale, extra_input_scaling
):
...
def main():
    ...
    writer = (
        None
        if arg.no_tensorboard
        else SummaryWriter(log_dir=os.path.join(arg.log_dir, "tensorboard/"))
    )
    ...
    if (
        ckpt is not None
        and arg.n_features_in_enc is not None
        and arg.n_classes is not None
        and arg.n_split_layers is None
    ):
        n_classes = arg.n_classes if arg.n_classes > 0 else None
        n_features_in_enc = arg.n_features_in_enc
        train_loader = None
    else:
        train_loader, n_classes = load_dataset(
            arg.dataset,
            arg.image_size,
    ...
    train_enc_input_scalings = get_enc_input_scalings(
        arg.net_enc_type,
        arg.dataset,
        arg.image_size,
        arg.data_scale,
    ...
    encoders = get_torchvision_encoders(
        arg.net_enc_type,
        arg.image_size,
        device,
        arg.pretrain_dataset,
        arg.n_classes_in_enc,
        arg.n_split_layers,
        n_classes,
        train_enc_input_scalings,
        arg.dataset,
    )

    if arg.val_enc is not None:
        val_encoders = get_torchvision_encoders(
            arg.val_enc,
            arg.image_size,
            device,
            "imagenet",
    ...
    noise_maker = GeneratorNoiseMaker(
        arg.batch_size, arg.z_dim, device, n_classes, ckpt
    )
    ...
    m_avg.mean, m_avg.var = get_mean_and_var_nets(
        n_features_in_enc, device, ckpt, n_classes
    )
    ...
    for step in range(arg.first_batch_id + 1, event_steps.final + 1):
        ...
        if (
            arg.pretrain_dataset == "imagenet"
            and arg.gen_output == "tanh"
            and arg.data_scale == "normed"
        ):
            # normalizes the generated images using imagenet min-max ranges
            imagenet_norm_min, imagenet_norm_range = get_imagenet_norm_min_and_range(
                device
            )
            fake_data = (
                ((fake_data + 1) * imagenet_norm_range) / 2
            ) + imagenet_norm_min

        # extract features from FAKE data
        feat_emb = get_fake_data_embedding(
            fake_data,
            encoders,
            feat_emb,
            dp_params,
        ...
        adam_moving_average_update(
            feat_emb, acc_losses, m_avg, optimizers, arg.matched_moments
        )
        ...
        if step % event_steps.eval == 0:  # note this is all train loss, not validation!
            ...
            lm, lv = get_static_losses_valid(
                feat_emb.means_valid,
                feat_emb.vars_valid,
                gen,
                val_encoders,
                noise_maker,
            ...
            syn_data_file, eval_score = log_synth_data_eval(
                gen,
                writer,
                step,
                noise_maker,
                ...
                arg.log_dir,
                n_classes,
                arg.fid_dataset_size,
                arg.image_size,
                arg.center_crop_size,
                arg.data_scale,
                arg.local_fid_eval_storage,
                arg.skip_prdc,
                final_step=False,
            )

            update_best_score(eval_score, step, syn_data_file, arg.dataset, best_result)
            update_best_proxy_score(
                static_val_loss, step, syn_data_file, best_proxy_result
            )

            old_syn_data_file = delete_old_syn_data(
                syn_data_file,
                old_syn_data_file,
                best_proxy_result,
                step,
        ...
        if step % event_steps.ckpt == 0:
            create_checkpoint(
                ...
                fixed_noise,
                arg.log_dir,
                event_steps.new_ckpt,
                best_result,
                best_proxy_result,
            )

        if (
            (event_steps.restart is not None)
            and (step % event_steps.restart == 0)
            and event_steps.restart > 0
            and step != event_steps.final
        ):
    ...
    if arg.synth_dataset_size is not None:
        ...
        update_best_score(
            eval_score, arg.n_iter, syn_data_file, arg.dataset, best_result
        )
        ...
        static_val_loss = lm + lv
        update_best_proxy_score(
            static_val_loss, arg.n_iter, syn_data_file, best_proxy_result
        )
        os.remove(syn_data_file)
